  ...
  return (
    <div style={{ padding: "24px 22px 32px" }}>
      ...
      >
        ...
        <Row
          icon="⬆"
          label="File dal telefono"
          onClick={() => fileInputRef.current?.click()}
          spin={false}
        />
        <div style={{ height: 1, background: "rgba(246,238,218,.14)", margin: "14px 0" }} />
        <div>
          <p style={{ fontSize: 14, fontWeight: 600, margin: "0 0 8px" }}>Incolla il testo</p>
          <textarea
            value={pastedText}
...
function Row({ icon, label, onClick, spin }: { icon: string; label: string; onClick: () => void; spin: boolean }) {
  return (
    <button
      type="button"
      onClick={onClick}
